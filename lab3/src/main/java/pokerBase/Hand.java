...
public class Hand{
	...
	public ArrayList<Hand> CreateHandsFor1Joker(Hand h, int jokerIndex){
		ArrayList<Hand> HandList = new ArrayList<Hand>();
		for (eRank rank:eRank.values()){
			Card newCard = new Card(eSuit.JOKER,rank,rank.getiRankNbr());
			ArrayList<Card> newhandList = h.getCardsInHand(); 
			newhandList.set(jokerIndex,newCard);
			Hand newHand = new Hand();
			newHand.setCardsInHand(newhandList);
			HandList.add(newHand);
		}
		return HandList;
	}
	
	public ArrayList<Hand> CreateAllPossibleHands(Hand h){
		int NumJokers = 0;
		...
		AllPossibleHands.add(h);
		ArrayList<Hand> NextIter = new ArrayList<Hand>();
		while (NumJokers > 0){
			NumJokers -= 1;
			for (Hand h1:AllPossibleHands){
				for (Card c:h1.getCardsInHand()){
					if (c.geteRank() == eRank.JOKER){
						int JokerIndex = h1.getCardsInHand().indexOf(c);
						ArrayList<Hand> next = CreateHandsFor1Joker(h1,JokerIndex);
						for (Hand h2:next){
							NextIter.add(h2);
			}}}}
			if (NextIter != null){AllPossibleHands = NextIter;}
		}
		for (Hand H: AllPossibleHands){
			try {
				H = EvaluateHand(H);
	...
	public Hand GetBestHand(ArrayList<Hand> HandList) throws HandException{
		int HighestScore = 0;
		Hand highestHand = new Hand();
		for (Hand h : HandList){
			if (h.getHandScore().getHandStrength() > HighestScore){
				HighestScore = h.getHandScore().getHandStrength();
				highestHand = h;
			} else if(h.getHandScore().getHandStrength() == HighestScore){
				throw new HandException(h);
			}
		}
		return highestHand;
